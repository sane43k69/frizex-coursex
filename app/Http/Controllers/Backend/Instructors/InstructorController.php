    ...
    public function frontShow($id)
{
    ...
    // Загружаем курсы с данными инструктора + подсчётом уроков и студентов
    $courses = \App\Models\Course::where('instructor_id', $instructor->id)
        ->with(['instructor']) // подгружаем инструктора
        ->withCount(['lesson', 'enrollment'])
        ->get();

    return view('frontend.instructorProfile', compact('instructor', 'courses'));
}
